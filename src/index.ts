...
const appendWriteFile = async (filePath: string, content: string): Promise<void> => {
...
const getMessageQueue = (parsed: ReturnType<typeof parseFile>, batchSize = 15) => {
    // console.log("getMessageQueue parsed param : " + JSON.stringify(parsed));
    const cues = parsed.map(({
                                 type,
                                 code,
                                 data
                             }) => type === 'cue' && !!data?.text ? `<${code}>\n${data.text}` : null).filter(Boolean) as string[]
...
const translateQueue = async (queue: string[], progress: number) => {
    const prompt = options.prompt.replace('[lang]', options.language).replace('[text]', queue.join(`\n<000000>\n`))
    // console.log(">>>>>>> prompt : " + prompt);
    let translation = null
    while (!(translation = await getChatGPTAnswer(prompt))) {
        console.debug(`Translating queue ${progress} failed, retry...`)
    }
    return translation.replace(/^\s*(.*)\s*$/gm, '$1').split('<000000>');
}
const translateManager = async (queue: string[],progress:number) => {
    const result: string[] = []
    const queueResult = await translateQueue(queue, progress)
    if (queueResult !== queue)
        console.debug(`Translating queue ${progress} ... done!`)
    else
        console.debug(`Translating queue ${progress} ... failed! (skipping)`)
    result.push(...queueResult)
    return result
}
const translatedToCues = (translated: string[]) => {
    console.debug('Converting translated cues to objects...')
    const cues = translated.filter(item => item != undefined && item != '')
        .map(item => {
            try {
                item = item.replace(/\n/g, '');
                const [_, code, text] = item.matchAll(/<([A-Z0-9]{6})>(.*)/gm).next().value
                if (!code || !text) return null
                return {code, text}
            } catch (e) {
                console.error(">>>>>> 异常之后item: " + item);
                console.error(">>>>>> 异常之后 : " ,e);
                throw e
            }
        }).filter(Boolean)
...
const main = async () => {
    ...
    const translatedCues:any[] = [];
    for (const queueIndex in messages) {
        const progress = parseInt(queueIndex, 10) + 1
        const queue = messages[queueIndex]
        const length = messages.length
        console.debug(`Translating queue ${progress} of ${length}...`)
        const translateResult = await translateManager(queue,progress)
        console.log(inspect(translateResult, {depth: null, colors: true}))
        const translatedCue = translatedToCues(translateResult)
    ...
    await appendWriteFile(outputFile, output)
    console.log(inspect(`Translating queues success`, {depth: null, colors: true}))
}
